...
/* Code must never be stale — a cached admin.js after an update means the
   panel silently runs yesterday's logic. Pictures can cache happily. */
const LONG_CACHE = new Set(['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.ico', '.woff2']);

function cacheFor(filePath) {
  return LONG_CACHE.has(path.extname(filePath).toLowerCase())
...
function serveFile(res, filePath, { cache } = {}) {
  ...
  res.writeHead(200, {
    ...
    'Cache-Control': cache ?? cacheFor(filePath),
  });
  fs.createReadStream(filePath).pipe(res);
  return true;
}

module.exports = { MIME, escapeHtml, sendJson, sendText, readJsonBody, resolveInside, serveFile, cacheFor };
