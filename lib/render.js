...
function orderButtons(urls, label, price, currency) {
  const message = price
    ? `Hi! I'd like to order the ${label} (${currency}${price}). Is it available?`
    : `Hi! I'd like to order the ${label}.`;
...
function productCard(p, s, urls) {
  ...
  return `      <article class="card reveal" data-product="${escapeHtml(p.name)}" data-price="${escapeHtml(p.price)}">
        <div class="shot">
        ...
        <div class="card-actions">
          ${orderButtons(urls, p.name, p.price, s.currency)}
        </div>
      </article>`;
}
  ...
  return `    <div class="bundle reveal" data-product="${escapeHtml(s.bundleTitle)}" data-price="${escapeHtml(s.bundlePrice)}">
      <div class="bundle-in">
        ...
        <div class="card-actions bundle-actions">
          ${orderButtons(urls, s.bundleTitle, s.bundlePrice, s.currency)}
        </div>
      </div>
    </div>`;
}
